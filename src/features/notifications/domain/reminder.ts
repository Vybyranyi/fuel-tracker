 ...
 *
 * Авто перелічені в тексті, а не рознесені по окремих сповіщеннях: три авто —
 * це три однакові банери підряд, які привчають змахувати не читаючи.
 */
export function odometerReminder(
  month: MonthKey,
  ...
  return {
    ...
    body:
      carNames.length > 0
        ? `Внеси показання одометра: ${carNames.join(", ")}.`
        : "Місяць закінчується — внеси показання одометра.",
    url: "/odometer",
